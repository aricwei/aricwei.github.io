...
The balls are 10 mm in diameter. In theory, with a 10 mm screw shaft inner diameter (ID) and 30 mm screw shaft outer diameter (OD), there should be room for exactly six balls. In reality, due to gravity and friction from the 45 degrees incline, I would try out 1-3 balls per pitch length. I set the pitch to be pitch_screw = `2/3*OD_screw` = `2/3*30 mm` = 20 mm. Screw conveyor engineering guides that I have read like [KWS Manufacturing](https://www.kwsmfg.com/engineering-guides/screw-conveyor/ types-of-screw-conveyors/), recommend reduced pitch for 45 degrees incline. In my case that only affects capacity if I were to fit more than one 10 mm ball side by side in between pitch lengths.
<br>
<br> I adapted M4 X 16 socket head cap screws for the feet. I turned them upside and fitted into a ball and socket part for rotational adjustment. For height adjustment I used two nuts to adjust and tighten it to the base crossmembers.
